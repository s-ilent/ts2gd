...
const containsAwaitExpression = (node: ts.Node): boolean =>
  ts.isAwaitExpression(node) || node.getChildren().some(containsAwaitExpression)
  ...
  const visit = (node: ts.Node): void => {
    if (
      ...
      if (symbol && !bindings.has(symbol)) {
        ...
        // A hoisted body containing await is a GDScript coroutine; call
        // sites must await it or they receive a suspended state object.
        const isCoroutine = containsAwaitExpression(node.body)
...
export const testNestedCoroutineCallAwaits: Test = {
  ts: `
export function runSession(force: boolean) {
  let attempts = 0
  async function runCheck(f: boolean): Promise<boolean> {
...
static func __nested_runCheck(f: bool, captures):
  var attempts = captures.attempts
  if attempts > 2:
    return false
...
static func runSession(force: bool):
  var attempts: int = 0
  
  return await __nested_runCheck(force, {"attempts": attempts})
