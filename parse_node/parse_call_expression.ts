...
const plainCalleeName = (
  ...
  if (
    ...
    return mangleGdName(decl.name.text)
  }
...
export const parseCallExpression = (
  ...
  if (expression.kind === SyntaxKind.Identifier) {
    ...
    if (
      ...
      const funcName = mangleGdName(decl.name.text)
  ...
  if (node.expression.kind === SyntaxKind.PropertyAccessExpression) {
    ...
    // JS Map/Set get/set calls route to the shim spellings (bare get/set
    // would clash with Godot's Object methods at runtime). A one-arg
    // set() whose receiver resolves no project member is the typed-array
    // bulk copy (Float32Array.set and friends), which Packed arrays lack.
    if (
      ts.isPropertyAccessExpression(node.expression) &&
      (functionName === "set" || functionName === "get")
    ) {
      const receiverTypeString = props.program
        .getTypeChecker()
        .typeToString(
          props.program.getTypeChecker().getTypeAtLocation(prop.expression)
        )

      // With noLib the collection constructors resolve no declaration, so
      // receivers may type as error even when they are unambiguously maps
      // or sets; producing expressions and variable initializers identify
      // them structurally.
      const producedByCollection = ((expr: ts.Expression): boolean => {
        const isNewCollection = (candidate: ts.Expression): boolean =>
          ts.isNewExpression(candidate) &&
          ts.isIdentifier(candidate.expression) &&
          /^(?:Weak)?(?:Map|Set)$/.test(
            (candidate.expression as ts.Identifier).text
          )

        if (isNewCollection(expr)) {
          return true
        }

        if (ts.isIdentifier(expr)) {
          const decl = props.program.getTypeChecker().getSymbolAtLocation(expr)
            ?.declarations?.[0]
          ...
          if (
            ...
            decl.initializer &&
            isNewCollection(decl.initializer)
          ) {
            return true
          }
        }
      ...
      if (
        args.length === (functionName === "set" ? 2 : 1) &&
        (producedByCollection ||
          (/^(?:Readonly)?(?:Weak)?(?:Set|Map)\b/.test(receiverTypeString) &&
            true))
      ) {
        return combine({
          parent: node,
          ...
          parsedStrings: (expr, ...parsed) =>
            `${expr}.ts_${functionName}(${parsed.join(", ")})`,
        })
      }
      ...
      const receiverMemberDeclarations =
        props.program.getTypeChecker().getSymbolAtLocation(prop.name)
          ?.declarations ?? []
      const receiverIsUserMember = receiverMemberDeclarations.some(
      ...
      if (
        ...
        const result = combine({
          ...
          props,
          parsedStrings: (expr, ...parsed) =>
            `__ts_typed_array_set(${[expr, ...parsed].join(", ")})`,
        })

        result.hoistedLibraryFunctions =
          result.hoistedLibraryFunctions ?? new Set()
        result.hoistedLibraryFunctions.add("ts_typed_array_set")
    ...
    // Static-class helpers receive no receiver argument: String/Array/etc.
    // are builtin type names in GDScript and cannot be used as expressions.
    const staticHelperCall = (
      libName: LibraryFunctionName,
      wrapArgsInArray = false
    ) => {
      const result = combine({
        ...
        parsedStrings: (...parsed) =>
          `__${libName}(${
            wrapArgsInArray ? "[" + parsed.join(", ") + "]" : parsed.join(", ")
          })`,
      })
    ...
    if (
      ...
      // Static class helper: the receiver is the global String constructor,
      // which has no value form in GDScript (builtin type names cannot stand
      // alone), so only the arguments are forwarded.
      return staticHelperCall("ts_string_from_char_code", true)
    }
    ...
    if (functionName === "from" && prop.expression.getText() === "Array") {
      // Static class helper: Array (the constructor) is not a GDScript
      // value, so the receiver is dropped. Array.from accepts array-like
      // objects ({ length: n }) plus an optional mapping callback, and the
      // helper iterates indices itself in that case.
      const result = staticHelperCall("ts_array_from")
    ...
    // import.meta.glob(pattern, options) compiles to a hoisted helper that
    // stands in for Vite's build-time asset globbing.
    if (
      functionName === "glob" &&
      prop.expression.kind === SyntaxKind.MetaProperty
    ) {
      return staticHelperCall("ts_glob")
    }
...
static func _static_init():
  __map(x, [Callable(__Mod_Test_4064or, "__gen"), {"z": z, "big": big}])
`,
}
...
static func _static_init():
  __map(__filter(a, [Callable(__Mod_Test_4064or, "__gen"), {}]), [Callable(__Mod_Test_4064or, "__gen1"), {}])
`,
}
...
static func _static_init():
  __map(x, [Callable(__Mod_Test_4064or, "__gen"), {"big": big}])
`,
}
...
static func _static_init():
  __ts_array_sort(nums, [Callable(__Mod_Test_4064or, "__gen2"), {}])
`,
}
...
static func pick(slot: float):
  return (table.ts_get("k").call(slot) if (table.ts_get("k").call(slot)) != null else null)
`,
}
...
export const testStringFromCharCodeDropsReceiver: Test = {
  ts: `
export class Test {
  static letter(n: number): string {
  ...
  expected: `
class_name Test
${LibraryFunctions.ts_string_from_char_code.definition(
  "__ts_string_from_char_code"
)}
static func letter(n: float):
  return __ts_string_from_char_code([97 + n])
  `,
}

export const testArrayFromObjectShapeDropsReceiver: Test = {
  ts: `
export class Test {
  static slots(n: number) {
  ...
  expected: `
class_name Test
${LibraryFunctions.ts_array_from.definition("__ts_array_from")}
${LibraryFunctions.ts_call_fn.definition("__ts_call_fn")}
static func __gen(captures):
  return 0


static func slots(n: float):
  return __ts_array_from({ "length": n }, [Callable(Test, "__gen"), {}])
  `,
}

export const testImportMetaGlobMapsToHelper: Test = {
  ts: `
const assets = import.meta.glob('../../data/quests/*/*', { eager: true })
export class Test {
  count() {
...
${LibraryFunctions.ts_glob.definition("__ts_glob")}
static var assets = __ts_glob("../../data/quests/*/*", { "eager": true })
func count():
...
export const testCollectionAndTypedArraySetCalls: Test = {
  ts: `
export class Test {
  static cache() {
    const clocks = new Map<number, number>()
    clocks.set(1, 30)
    const v = clocks.get(1)
    print(v)
  }
  static blend(local) {
  ...
  static weakClocks() {
    const clocks = new WeakMap<number, number>()
    clocks.set(1, 30)
    print(clocks.get(1))
  }
}
...
static func cache():
  var clocks = __ts_new_map()
  
  clocks.ts_set(1, 30)
  var v = clocks.ts_get(1)
  
  print(v)
static func blend(local):
  var matrix = __ts_new_float32(12)
  
  __ts_typed_array_set(matrix, local)
static func weakClocks():
  var clocks = __ts_new_weak_map()
