...
const walkFreeVariableChildren = (
  ...
  ts.forEachChild(node, (child) => {
    ...
    // Destructuring patterns declare locals; they contain no variable
    // references. A renamed binding element's property NAME (`{ hp: restoreHp }`)
    // resolves to the source property's symbol, whose declaration lives in a
    // type — frequently a sibling function's return annotation — so walking it
    // reports a phantom free variable and puts an undeclared name into the
    // capture dict. The element's initializer (its only value position) is
    // still walked.
    if (ts.isBindingElement(node)) {
      const element = node as ts.BindingElement

      if (child === element.propertyName || child === element.name) {
        return
      }
    }
...
export const testRenamedDestructuringLeavesNoPhantomCaptures: Test = {
  ts: `
function restore(): { hp: number; tp: number } {
  return { hp: 5, tp: 3 }
}
...
export class Test {
  static system() {
    function take(): number {
      const { hp: gotHp, tp: gotTp } = restore()
      return gotHp + gotTp
    }
...
static func __nested_take(captures):
  var __gen = restore()
  var gotHp = __gen.hp
  var gotTp = __gen.tp
