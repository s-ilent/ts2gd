...
export const getImportResPathForEnum = (
  ...
  // The compiling asset's fsPath is project-relative while the checker
  // spells source file names absolutely, so the self-referential case
  // compares by suffix; registry entries match either spelling.
  const enumSourceFileAsset =
    props.project
      .sourceFiles()
      .find((sf) => sf.fsPath === enumSourceFile.fileName) ??
    (enumSourceFile.fileName.endsWith(
      "/" + props.sourceFileAsset.fsPath.replace(/^\.\//, "")
    )
...
/**
 * Vite's `?raw` suffix imports the file's TEXT CONTENT, not its URL. The
 * binding must therefore read the asset at runtime; a bare res:// path
 * string would leak into consumers like JSON.parse as garbage input.
 */
const isRawAssetQuery = (modulePath: string) =>
  modulePath.split("?").slice(1).join("?").split("&").includes("raw")

/**
 * The runtime expression an asset-query import binds its name to. `?url`
 * (and unknown queries) keep the path string; `?raw` reads the file so the
 * name holds the asset's text content, matching Vite's semantics.
 */
const assetBindingExpression = (
  moduleSpecifier: string,
  assetResPath: string
): string =>
  isRawAssetQuery(moduleSpecifier)
    ? `FileAccess.get_file_as_string("${assetResPath}")`
    : `"${assetResPath}"`
...
export const testRawAssetImportReadsFileContent: Test = {
  ts: `
import raw from "./data/level.json?raw"
