...
declare class XRInterfaceExtension extends XRInterface {
  ...
  protected _get_projection_for_view(
    view: int,
    aspect: float,
    z_near: float,
    z_far: float
  ...
  protected _get_suggested_pose_names(
  ...
  protected _get_transform_for_view(
    view: int,
  ...
  protected _get_vrs_texture_format(): int
  ...
  protected _trigger_haptic_pulse(
    action_name: string,
    tracker_name: StringName,
    frequency: float,
    amplitude: float,
  ...
  connect<T extends SignalsOf<XRInterfaceExtension>>(
    signal: T,
    method: SignalFunction<XRInterfaceExtension[T]>
  ): number
}
