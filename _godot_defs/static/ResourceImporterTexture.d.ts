...
declare class ResourceImporterTexture extends ResourceImporter {
   ...
   *
   * - In 2D, a [CanvasItemMaterial] will need to be created and configured to use the [constant CanvasItemMaterial.BLEND_MODE_PREMULT_ALPHA] blend mode on [CanvasItem]s that use this texture. In custom `@canvas_item` shaders, `render_mode blend_premul_alpha;` should be used.
   *
   * - In 3D, a [BaseMaterial3D] will need to be created and configured to use the [constant BaseMaterial3D.BLEND_MODE_PREMULT_ALPHA] blend mode on materials that use this texture. In custom `spatial` shaders, `render_mode blend_premul_alpha;` should be used.
   *
   */
   ...
   *
   * **Note:** Even if this is set to `0`, import size is limited to the following dimensions for technical reasons. Depending on [member compress/mode], textures will be downsampled on import if necessary:
   *
  ...
  connect<T extends SignalsOf<ResourceImporterTexture>>(
    signal: T,
    method: SignalFunction<ResourceImporterTexture[T]>
  ): number
}
