/**
 * A color represented in RGBA format by a red ([member r]), green ([member g]), blue ([member b]), and alpha ([member a]) component. Each component is a 32-bit floating-point value, usually ranging from `0.0` to `1.0`. Some properties (such as [member CanvasItem.modulate]) may support values greater than `1.0`, for overbright or HDR (High Dynamic Range) colors.
 *
...
declare class ColorConstructor {
   ...
   *
   * **Note:** The alpha channel is always stored with linear encoding, regardless of the encoding of the other color channels. The [method linear_to_srgb] and [method srgb_to_linear] methods do not affect the alpha channel.
   *
   */
  a: float
  ...
  connect<T extends SignalsOf<Color>>(
    signal: T,
    method: SignalFunction<Color[T]>
  ): number
